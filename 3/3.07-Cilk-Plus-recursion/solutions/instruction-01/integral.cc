...
This file, labs/3/3.07-Cilk-Plus-recursion/solutions/instruction-01/integral.cc,
...
double recursive_integral(const long minIter, const long maxIter, const double dx) {  
  ...
  if(chunk < chunk_threshold) {
    ...
    double child_integral = _Cilk_spawn recursive_integral(minIter, minIter+chunk/2L, dx);  
